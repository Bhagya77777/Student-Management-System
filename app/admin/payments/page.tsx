...
import { Loader2, Download, DollarSign, Clock, TrendingUp } from "lucide-react";
...
export default function AdminPaymentsPage() {
  ...
  const chartData = useMemo(
    () => payments.map((payment) => ({ name: payment.category, amount: payment.amount })),
    [payments]
  );
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-slate-50 via-white to-blue-50/30">
      <div className="flex h-screen overflow-hidden">
        <DashboardSidebar items={adminSidebarItems} activeItem="payments" />
          ...
          <main className="flex-1 overflow-auto">
            <div className="p-6 space-y-6">
              <div>
                ...
                <p className="text-gray-500 text-sm mt-1">Monitor revenue, transactions, and payment trends</p>
              </div>

              <div className="grid grid-cols-4 gap-4">
                <Card className="border-0 shadow-sm"><CardContent className="p-4"><p className="text-xs text-gray-500">Revenue</p><p className="text-2xl font-bold text-green-600">LKR {totalRevenue.toLocaleString()}</p></CardContent></Card>
                <Card className="border-0 shadow-sm"><CardContent className="p-4"><p className="text-xs text-gray-500">Due</p><p className="text-2xl font-bold text-red-600">LKR {totalDue.toLocaleString()}</p></CardContent></Card>
                <Card className="border-0 shadow-sm"><CardContent className="p-4"><p className="text-xs text-gray-500">Pending</p><p className="text-2xl font-bold text-orange-600">{pendingCount}</p></CardContent></Card>
                <Card className="border-0 shadow-sm"><CardContent className="p-4"><p className="text-xs text-gray-500">Transactions</p><p className="text-2xl font-bold text-gray-900">{payments.length}</p></CardContent></Card>
              </div>
              ...
              <Card className="border-0 shadow-sm">
                ...
                <CardContent>
                  <div className="space-y-3">
                    {payments.map((payment) => (
                      <div key={payment.id} className="rounded-xl border border-gray-100 p-4 flex items-center justify-between gap-4">
                        <div>
                          ...
                          <p className="text-sm text-gray-500">{payment.studentName} {payment.studentCode ? `(${payment.studentCode})` : ""}</p>
                        </div>
                    ...
                    {payments.length === 0 && <div className="rounded-xl border border-dashed border-gray-200 p-6 text-sm text-gray-500">No payments found.</div>}
                  </div>
                </CardContent>
              </Card>
            </div>
